...
void MAX30100::setLEDs(pulseWidth pw, ledCurrent red, ledCurrent ir){
    ...
    data_write[0] = (red<<4);
    i2c_write(MAX30100_ADDRESS, MAX30100_LED_CONFIG_1, data_write, 1); // write LED configs
    data_write[0] = (ir<<4);
    i2c_write(MAX30100_ADDRESS, MAX30100_LED_CONFIG_2, data_write, 1); // write LED configs
}

void MAX30100::setFIFO(sampleAvg avg){
    char data_write[1];
    data_write[0] = 0x40;    // enable data ready interrupt
    i2c_write(MAX30100_ADDRESS, MAX30100_INT_ENABLE, data_write, 1);
    data_write[0] = (avg<<5) | 0x10;    // set data averaging
    i2c_write(MAX30100_ADDRESS, MAX30100_FIFO_CONFIG, data_write, 1);
}
...
int MAX30100::getDataRdyStatus(void){
    char data_read[1];
...
void MAX30100::readSensor(void){
    char data_read[3];
    i2c_read(MAX30100_ADDRESS, MAX30100_FIFO_DATA_REG, data_read, 3);  // Read three times from the FIFO
...
void MAX30100::begin(pulseWidth pw, ledCurrent ir, sampleRate sr){
    ...
    data_write[0] = ir;
    i2c_write(MAX30100_ADDRESS, MAX30100_LED_CONFIG_2, data_write, 1);
...
void MAX30100::printRegisters(void){
    ...
    i2c_read(MAX30100_ADDRESS, MAX30100_FIFO_CONFIG, data_read, 1);
    pc.printf("FIFO_CONFIG: %#4X\r\n", data_read[0]);
    i2c_read(MAX30100_ADDRESS, MAX30100_CONFIG, data_read, 1);
    pc.printf("CONFIG: %#4X\r\n", data_read[0]);
    i2c_read(MAX30100_ADDRESS, MAX30100_SPO2_CONFIG, data_read, 1);
    pc.printf("SPO2_CONFIG: %#4X\r\n", data_read[0]);
    i2c_read(MAX30100_ADDRESS, MAX30100_LED_CONFIG_1, data_read, 1);
    pc.printf("LED_CONFIG_1: %#4X\r\n", data_read[0]);
    i2c_read(MAX30100_ADDRESS, MAX30100_LED_CONFIG_2, data_read, 1);
